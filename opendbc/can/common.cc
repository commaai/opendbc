...
unsigned int volkswagen_mqb_checksum(uint32_t address, const Signal &sig, const std::vector<uint8_t> &d) {
  ...
  switch (address) {
    ...
    case 0x120: // TSK_06 Drivetrain Coordinator
      ...
      break;
    case 0x31E: // TSK_07 Drivetrain Coordinator
    ...
    case 0x187: // EV_Gearshift "Gear" selection data for EVs with no gearbox
      ...
      break;
    case 0x2A9: // ACC_15 Front Collision Warning
...
unsigned int volkswagen_meb_checksum(uint32_t address, const Signal &sig, const std::vector<uint8_t> &d) {
  // Volkswagen uses standard CRC8 8H2F/AUTOSAR, but they compute it with
  // a magic variable padding byte tacked onto the end of the payload.
  // https://www.autosar.org/fileadmin/user_upload/standards/classic/4-3/AUTOSAR_SWS_CRCLibrary.pdf
  ...
  switch (address) {
    ...
    case 0xFD:  // ESP_21 Electronic Stability Program
      crc ^= (uint8_t[]){0xB4, 0xEF, 0xF8, 0x49, 0x1E, 0xE5, 0xC2, 0xC0, 0x97, 0x19, 0x3C, 0xC9, 0xF1, 0x98, 0xD6, 0x61}[counter];
      break;
    case 0x3BE: // Motor_14 Driver Brake Inputs

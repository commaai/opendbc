...
static const std::unordered_map<uint32_t, std::array<uint8_t, 16>> volkswagen_mxb_crc_constants {
...
unsigned int volkswagen_mxb_checksum(uint32_t address, const Signal &sig, const std::vector<uint8_t> &d) {
  ...
  auto crc_const = volkswagen_mxb_crc_constants.find(address);
  if (crc_const != volkswagen_mxb_crc_constants.end()) {

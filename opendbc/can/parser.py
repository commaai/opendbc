  ...
  def can_valid(self) -> bool:
    ...
    for state in self.message_states.values():
      if state.counter_fail >= MAX_BAD_COUNTER:
        ...
        carlog.error({"counter invalid - address": hex(self.message_states.address), "name": self.message_states.name, "bus": self.message_states.bus})
      ...
      if not state.valid(self._last_update_nanos, bus_timeout):
        valid = False
        carlog.error({"can invalid - address": hex(self.message_states.address), "name": self.message_states.name, "bus": self.message_states.bus})

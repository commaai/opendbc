...
#define HYUNDAI_COMMON_RX_CHECKS(legacy)                                                                                      \
  ...
  {.msg = {{0x386, 0, 8, .skip_checksum = (legacy), .max_counter = (legacy) ? 0U : 15U, .frequency = 100U}, { 0 }, { 0 }}}, \
  {.msg = {{0x394, 0, 8, .skip_checksum = (legacy), .max_counter = (legacy) ? 0U : 7U, .frequency = 100U}, { 0 }, { 0 }}},  \
...
static safety_config hyundai_init(uint16_t param) {
  ...
  if (hyundai_longitudinal) {
    static RxCheck hyundai_long_rx_checks[] = {
      ...
      {.msg = {{0x4F1, 0, 4, .skip_checksum = true, .max_counter = 15U, .frequency = 50U}, { 0 }, { 0 }}},
    };

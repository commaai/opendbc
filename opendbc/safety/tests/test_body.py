...
class TestBody(common.SafetyTest):
  ...
  def test_can_flasher(self):
    ...
    self.assertFalse(self._tx(common.make_msg(0, 0x250, dat=b'\xce\xfa\xad\xde\x2e\x0b\xb0\x0a'))) # wrong 0aboob1e
    self.assertFalse(self._tx(common.make_msg(0, 0x250, dat=b'\xce\xfa\xad\xde\x1e\x0b\xb0')))  # not correct len
    self.assertFalse(self.safety._test_tx_hook(common.make_msg(0, 0x251, dat=b'\xce\xfa\xad\xde\x1e\x0b\xb0\x0a')))  # wrong address

...
from opendbc.car import get_max_angle_delta_vm, get_max_angle_vm
...
class TestTeslaSafetyBase(common.PandaCarSafetyTest, common.AngleSteeringSafetyTest, common.LongitudinalAccelSafetyTest):
  ...
  def _get_steer_cmd_angle_max(self, speed):
    return get_max_angle_vm(max(speed, 1), self.VM)
  ...
  def test_lateral_accel_limit(self):
    for speed in np.linspace(0, 40, 100):
      ...
      for sign in (-1, 1):
        ...
        max_angle = round_angle(get_max_angle_vm(speed, self.VM), angle_unit_offset + 1) * sign
        ...
        # 1 unit above limit
        max_angle_raw = round_angle(get_max_angle_vm(speed, self.VM), angle_unit_offset + 2) * sign
  ...
  def test_lateral_jerk_limit(self):
    for speed in np.linspace(0, 40, 100):
      ...
      for sign in (-1, 1):  # (-1, 1):
        ...
        # Stay within limits
        # Up
        max_angle_delta = round_angle(get_max_angle_delta_vm(speed, CarControllerParams.STEER_STEP, self.VM), angle_unit_offset) * sign
        ...
        # Up
        max_angle_delta = round_angle(get_max_angle_delta_vm(speed, CarControllerParams.STEER_STEP, self.VM), angle_unit_offset + 1) * sign

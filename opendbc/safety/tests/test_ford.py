...
class TestFordSafetyBase(common.CarSafetyTest):
  ...
  def _lat_ctl_msg(self, enabled: bool, path_offset: float, path_angle: float, curvature: float, curvature_rate: float,
                   mode: int | None = None, increment_timer: bool = True):
    if increment_timer:
  ...
  def test_canfd_path_mode(self):
    ...
    for controls_allowed in (True, False):
      for mode in range(8):
        self.safety.set_controls_allowed(controls_allowed)
        should_tx = mode == 0 or (mode == 1 and controls_allowed)
        self.assertEqual(should_tx, self._tx(self._lat_ctl_msg(mode == 1, 0., 0., 0., 0., mode=mode)))

    self.safety.set_controls_allowed(True)
    self.assertFalse(self._tx(self._lat_ctl_msg(False, 0.5, 0.1, 0., 0., mode=0)))
    self._reset_curvature_measurement(0., 5.)
    self._set_prev_desired_angle(0.01)
    self.assertTrue(self._tx(self._lat_ctl_msg(True, 0., 0., 0.01, 0., mode=1)))
    self.assertFalse(self._tx(self._lat_ctl_msg(True, 0.5, 0., 0., 0., mode=1)))
    self.assertFalse(self._tx(self._lat_ctl_msg(True, 0., 0.1, 0., 0., mode=1)))
    self.assertFalse(self._tx(self._lat_ctl_msg(True, 0., 0., 0., 0.001, mode=1)))

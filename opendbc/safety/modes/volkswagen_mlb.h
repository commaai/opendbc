...
static safety_config volkswagen_mlb_init(uint16_t param) {
  // Transmit of LS_01 is allowed on bus 0 and 2 to keep compatibility with
  // gateway and camera integration
  ...
  static RxCheck volkswagen_mlb_rx_checks[] = {
      ...
      {.msg = {{MSG_ESP_03, 0, 8, 50U, .ignore_checksum = true,
                .max_counter = 15U, .ignore_quality_flag = true},
               {0},
               {0}}},
      {.msg = {{MSG_LH_EPS_03, 0, 8, 100U, .ignore_checksum = true,
                .max_counter = 15U, .ignore_quality_flag = true},
               {0},
               {0}}},
      {.msg = {{MSG_ESP_05, 0, 8, 50U, .ignore_checksum = true,
                .max_counter = 15U, .ignore_quality_flag = true},
               {0},
               {0}}},
      {.msg = {{MSG_TSK_04, 1, 8, 50U, .ignore_checksum = true,
                .max_counter = 15U, .ignore_quality_flag = true},
               {0},
               {0}}},
      {.msg = {{MSG_MOTOR_03, 0, 8, 100U, .ignore_checksum = true,
                .max_counter = 15U, .ignore_quality_flag = true},
               {0},
               {0}}},
      {.msg = {{MSG_LS_01, 0, 4, 10U, .ignore_checksum = true,
                .max_counter = 15U, .ignore_quality_flag = true},
               {0},
               {0}}},
  };
  ...
  return BUILD_SAFETY_CFG(volkswagen_mlb_rx_checks,
                          VOLKSWAGEN_MLB_STOCK_TX_MSGS);
}
  ...
  if (msg->bus == 0U) {
    ...
    if (msg->addr == MSG_LH_EPS_03) {
      update_sample(&torque_driver,
                    volkswagen_mlb_mqb_driver_input_torque(msg));
    }
    ...
    brake_pressed =
        volkswagen_brake_pedal_switch || volkswagen_brake_pressure_detected;
  }

  if (msg->addr == MSG_TSK_04) {
    // When using stock ACC, enter controls on rising edge of stock ACC engage,
    // exit on disengage Signal: TSK_04.TSK_Status_GRA_ACC_02
...
static bool volkswagen_mlb_tx_hook(const CANPacket_t *msg) {
  ...
  const TorqueSteeringLimits VOLKSWAGEN_MLB_STEERING_LIMITS = {
      .max_torque =
          300, // 3.0 Nm (EPS side max of 3.0Nm with fault if violated)
      .max_rt_delta =
          169, // 10 max rate up * 50Hz send rate * 250000 RT interval / 1000000
               // = 112.5 ; 112.5 * 1.5 for safety pad = 168.75
      .max_rate_up =
          9, // 5.0 Nm/s RoC limit (EPS rack has own soft-limit of 5.0 Nm/s)
      .max_rate_down =
  ...
  if (msg->addr == MSG_HCA_01) {
    ...
    if (steer_torque_cmd_checks(desired_torque, steer_req,
                                VOLKSWAGEN_MLB_STEERING_LIMITS)) {
      tx = false;
    }
  }

  // FORCE CANCEL: ensuring that only the cancel button press is sent when
  // controls are off. This avoids unintended engagements while still allowing
  // resume spam

...
static void honda_rx_hook(const CANPacket_t *msg) {
  ...
  if ((msg->addr == 0x326) || (msg->addr == 0x1A6)) {
    acc_main_on = GET_BIT(msg, ((msg->addr == 0x326) ? 28U : 47U));
    if (!acc_main_on) {
  ...
  // state machine to enter and exit controls for button enabling
    ...
    if (((msg->addr == 0x1A6) || (msg->addr == 0x296)) && (msg->bus == pt_bus)) {
...
static bool honda_tx_hook(const CANPacket_t *msg) {
  ...
  if ((msg->addr == 0x30C) && (msg->bus == bus_pt)) {
  ...
  if ((msg->addr == 0x1FA) && (msg->bus == bus_pt)) {
  ...
  if ((msg->addr == 0x1DF) && (msg->bus == bus_pt)) {
  ...
  if ((msg->addr == 0x1C8) && (msg->bus == bus_pt)) {
  ...
  // STEER: safety check
  if ((msg->addr == 0xE4) || (msg->addr == 0x194)) {
    if (!controls_allowed) {
  ...
  if ((msg->addr == 0x296) && !controls_allowed && (msg->bus == bus_buttons)) {

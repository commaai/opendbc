...
static void tesla_rx_hook(const CANPacket_t *to_push) {
  ...
  if (bus == 0) {
    ...
    if (addr == 0x257) {
      ...
      float speed = kph_to_ms(((((GET_BYTE(to_push, 2) << 4) | (GET_BYTE(to_push, 1) >> 4)) * 0.08) - 40.));
    ...
    if (addr == 0x155) {
      ...
      float esp_speed = kph_to_ms((((GET_BYTE(to_push, 6) & 0x0FU) << 6) | GET_BYTE(to_push, 5) >> 2) * 0.5);
...
static safety_config tesla_init(uint16_t param) {
  ...
  static RxCheck tesla_model3_y_rx_checks[] = {
    ...
    {.msg = {{0x39d, 0, 5, .max_counter = 15U, .quality_flag = true, .frequency = 25U}, { 0 }, { 0 }}},  // IBST_status (brakes)

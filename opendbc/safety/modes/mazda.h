...
static void mazda_rx_hook(const CANPacket_t *msg) {
  if (ADDR_BUS_MATCH(msg, MAZDA_ENGINE_DATA, MAZDA_MAIN)) {
    // sample speed: scale by 0.01 to get kph
    int speed = (msg->data[2] << 8) | msg->data[3];
    vehicle_moving = speed > 10; // moving when speed > 0.1 kph
  }

  if (ADDR_BUS_MATCH(msg, MAZDA_STEER_TORQUE, MAZDA_MAIN)) {
  ...
  if (ADDR_BUS_MATCH(msg, MAZDA_CRZ_CTRL, MAZDA_MAIN)) {
  ...
  if (ADDR_BUS_MATCH(msg, MAZDA_PEDALS, MAZDA_MAIN)) {
...
static bool mazda_tx_hook(const CANPacket_t *msg) {
  ...
  // steer cmd checks
  if (ADDR_BUS_MATCH(msg, MAZDA_LKAS, MAZDA_MAIN)) {
  ...
  if (ADDR_BUS_MATCH(msg, MAZDA_CRZ_BTNS, MAZDA_MAIN)) {

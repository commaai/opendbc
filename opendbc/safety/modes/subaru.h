...
static void subaru_rx_hook(const CANPacket_t *msg) {
  ...
  if ((msg->addr == MSG_SUBARU_Steering_Torque) && (bus == SUBARU_MAIN_BUS)) {
  ...
  if ((msg->addr == MSG_SUBARU_CruiseControl) && (bus == alt_main_bus)) {
    bool cruise_engaged = GET_BIT(msg, 41U);
    pcm_cruise_check(cruise_engaged);
  }

  // update vehicle moving with any non-zero wheel speed
  if ((msg->addr == MSG_SUBARU_Wheel_Speeds) && (bus == alt_main_bus)) {
  ...
  if ((msg->addr == MSG_SUBARU_Brake_Status) && (bus == alt_main_bus)) {
    brake_pressed = GET_BIT(msg, 62U);
  }

  if ((msg->addr == MSG_SUBARU_Throttle) && (bus == SUBARU_MAIN_BUS)) {

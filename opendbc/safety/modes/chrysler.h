...
static void chrysler_rx_hook(const CANPacket_t *msg) {
  ...
  // Measured EPS torque
  if ((msg->bus == 0) && (msg->addr == chrysler_addrs->EPS_2)) {
  ...
  if ((chrysler_platform != CHRYSLER_PACIFICA) && (msg->bus == 0) && (msg->addr == chrysler_addrs->ESP_8)) {
    vehicle_moving = ((GET_BYTE(msg, 4) << 8) + GET_BYTE(msg, 5)) != 0U;
  }
  if ((chrysler_platform == CHRYSLER_PACIFICA) && (msg->bus == 0) && (msg->addr == 514)) {
  ...
  if ((msg->bus == 0) && (msg->addr == chrysler_addrs->ECM_5)) {
    gas_pressed = GET_BYTE(msg, 0U) != 0U;
  }

  // exit controls on rising edge of brake press
  if ((msg->bus == 0) && (msg->addr == chrysler_addrs->ESP_1)) {

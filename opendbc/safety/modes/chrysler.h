...
static void chrysler_rx_hook(const CANPacket_t *msg) {
  ...
  // Measured EPS torque
  if ((bus == 0) && (msg->addr == chrysler_addrs->EPS_2)) {
  ...
  if ((bus == das_3_bus) && (msg->addr == chrysler_addrs->DAS_3)) {
  ...
  if ((chrysler_platform != CHRYSLER_PACIFICA) && (bus == 0) && (msg->addr == chrysler_addrs->ESP_8)) {
    vehicle_moving = ((GET_BYTE(msg, 4) << 8) + GET_BYTE(msg, 5)) != 0U;
  }
  if ((chrysler_platform == CHRYSLER_PACIFICA) && (bus == 0) && (msg->addr == 514)) {
  ...
  if ((bus == 0) && (msg->addr == chrysler_addrs->ECM_5)) {
    gas_pressed = GET_BYTE(msg, 0U) != 0U;
  }

  // exit controls on rising edge of brake press
  if ((bus == 0) && (msg->addr == chrysler_addrs->ESP_1)) {

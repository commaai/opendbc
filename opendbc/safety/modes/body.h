...
static void body_rx_hook(const CANPacket_t *msg) {
  UNUSED(msg); // Note: since Rx check only has 1 message, we don't need to do anything here.
...
static bool body_tx_hook(const CANPacket_t *msg) {
  ...
  if (!controls_allowed && (GET_BYTES(msg, 0, 4) == 0xdeadfaceU) && (GET_BYTES(msg, 4, 4) == 0x0ab00b1eU)) {
    tx = true;
  }

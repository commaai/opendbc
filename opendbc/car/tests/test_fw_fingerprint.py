...
class TestFwFingerprint:
    def assertFingerprints(self, candidates, expected):
        ...
        assert len(
            candidates) == 1, f"got more than one candidate: {candidates}"
    ...
    def test_exact_match(self, brand, car_model, ecus, test_non_essential):
        ...
        for _ in range(20):
            ...
            CP.carFw = fw
            _, matches = match_fw_to_car(
                CP.carFw, CP.carVin, allow_fuzzy=False)
    ...
    def test_custom_fuzzy_match(self, brand, car_model, ecus):
        ...
        if config.match_fw_to_car_fuzzy is None:
            pytest.skip(
                "Brand does not implement custom fuzzy fingerprinting function")
        ...
        for _ in range(5):
            ...
            CP.carFw = fw
            _, matches = match_fw_to_car(
                CP.carFw, CP.carVin, allow_exact=False, log=False)
            brand_matches = config.match_fw_to_car_fuzzy(
                build_fw_dict(CP.carFw), CP.carVin, VERSIONS[brand])
        ...
        for brand, cars in VERSIONS.items():
            ...
            for ecus in cars.values():
                for ecu_type, addr, sub_addr in ecus.keys():
                    ...
                    ecu_strings = ", ".join(
                        [f'Ecu.{ecu}' for ecu in ecus_for_addr])
                    assert len(
                        ecus_for_addr) <= 1, f"{brand} has multiple ECUs that map to one address: {ecu_strings} -> ({hex(addr)}, {sub_addr})"
    ...
    def test_data_collection_ecus(self, subtests):
        ...
        for brand, config in FW_QUERY_CONFIGS.items():
            for car_model, ecus in VERSIONS[brand].items():
                ...
                with subtests.test(car_model=car_model.value):
                    assert not len(
                        bad_ecus), f'{car_model}: Fingerprints contain ECUs added for data collection: {bad_ecus}'
    ...
    def test_blacklisted_ecus(self, subtests):
        ...
        for car_model, ecus in FW_VERSIONS.items():
            with subtests.test(car_model=car_model.value):
                CP = interfaces[car_model][0].get_non_essential_params(
                    car_model)
                if CP.carName == 'subaru':
                    for ecu in ecus.keys():
                        assert ecu[
                            1] not in blacklisted_addrs, f'{car_model}: Blacklisted ecu: (Ecu.{ecu[0]}, {hex(ecu[1])})'
                    ...
                    if CP.carFingerprint.startswith("RAM_HD"):
                        for ecu in ecus.keys():
                            assert ecu[
                                0] != Ecu.transmission, f"{car_model}: Blacklisted ecu: (Ecu.{ecu[0]}, {hex(ecu[1])})"
    ...
    def test_missing_versions_and_configs(self, subtests):
        ...
        if len(brand_configs - brand_versions):
            with subtests.test():
                pytest.fail(
                    f"Brands do not implement FW_VERSIONS: {brand_configs - brand_versions}")

        if len(brand_versions - brand_configs):
            with subtests.test():
                pytest.fail(
                    f"Brands do not implement FW_QUERY_CONFIG: {brand_versions - brand_configs}")
    ...
    def test_fw_request_ecu_whitelist(self, subtests):
        for brand, config in FW_QUERY_CONFIGS.items():
            with subtests.test(brand=brand):
                whitelisted_ecus = {
                    ecu for r in config.requests for ecu in r.whitelist_ecus}
                brand_ecus = {fw[0] for car_fw in VERSIONS[brand].values()
                              for fw in car_fw}
        ...
        for brand, config in FW_QUERY_CONFIGS.items():
            with subtests.test(brand=brand):
                for request_obj in config.requests:
                    assert len(request_obj.request) == len(
...
class TestFwFingerprintTiming:
    ...
    def _benchmark_brand(self, brand, num_pandas, mocker):
        self.total_time = 0
        mocker.patch(
            "opendbc.car.isotp_parallel_query.IsoTpParallelQuery.get_data", self.fake_get_data)
    ...
    def _assert_timing(self, avg_time, ref_time):
        ...
        assert avg_time > ref_time - \
            self.TOL, "Performance seems to have improved, update test refs."

    def test_startup_timing(self, subtests, mocker):
        # Tests worse-case VIN query time and typical present ECU query time
        # best assumes we go through all queries to get a match
        ...
        self.total_time = 0.0
        mocker.patch("opendbc.car.fw_versions.get_ecu_addrs",
                     fake_get_ecu_addrs)
        for _ in range(self.N):
            self.current_obd_multiplexing = True
            get_present_ecus(self.fake_can_recv, self.fake_can_send,
                             self.fake_set_obd_multiplexing, num_pandas=2)
        self._assert_timing(self.total_time / self.N, present_ecu_ref_time)
        print(
            f'get_present_ecus, query time={self.total_time / self.N} seconds')
        ...
        for name, args in (('worst', {}), ('best', {'retry': 1})):
            with subtests.test(name=name):
                ...
                mocker.patch(
                    "opendbc.car.isotp_parallel_query.IsoTpParallelQuery.get_data", self.fake_get_data)
                for _ in range(self.N):
                    get_vin(self.fake_can_recv,
                            self.fake_can_send, (0, 1), **args)
                self._assert_timing(
                    self.total_time / self.N, vin_ref_times[name])
                print(
        ...
        for num_pandas in (1, 2):
            for brand, config in FW_QUERY_CONFIGS.items():
                with subtests.test(brand=brand, num_pandas=num_pandas):
                    ...
                    print(
                        f'{brand=}, {num_pandas=}, {len(config.requests)=}, avg FW query time={avg_time} seconds')
    ...
    def test_get_fw_versions(self, subtests, mocker):
        ...
        for brand in FW_QUERY_CONFIGS.keys():
            with subtests.test(brand=brand):
                get_fw_versions(self.fake_can_recv,
                                self.fake_can_send, lambda obd: None, brand)

...
def ford_checksum(address: int, sig, dat: bytearray) -> int:
  if address == 0x415:  # BrakeSysFeatures
    total = dat[0] + dat[1] + (dat[2] >> 6) + ((dat[2] >> 2) & 0xF)
  else:  # Yaw_Data_FD1
    total = sum(dat[:4]) + dat[5] + (dat[6] >> 6) + ((dat[6] >> 4) & 0x3)

  ...
  for i in range(retry):
    for bus in buses:
      for request, response, valid_buses, vin_addrs, functional_addrs, rx_offset in (
        ...
        tx_addrs = vin_addrs
        if functional_addrs is not None and i % 2 == 0:
          tx_addrs = [a for a in range(0x700, 0x800) if a != 0x7DF] + list(range(0x18DA00F1, 0x18DB00F1, 0x100))
        elif functional_addrs is not None:
          # Some ECUs answer a request addressed to them and stay silent on the functional one (the Hyundai Palisade
          # cluster at 0x7c6 is one). Attempts alternate: the functional request first, then the same request sent
          # directly to the standard VIN addresses, never both in one query so an ECU that answers both cannot put
          # two responses on one receiver.

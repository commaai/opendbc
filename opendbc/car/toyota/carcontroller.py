  ...
  def update(self, CC, CS, now_nanos):
    ...
    if self.CP.openpilotLongitudinalControl:
      if self.frame % 3 == 0:
        ...
        # calculate amount of acceleration PCM should apply to reach target, given pitch clipped to only
        # include downhill angles. avoids erroneously unsetting PERMIT_BRAKING when stopping on uphills
        accel_due_to_pitch = min(math.sin(self.pitch.x) * ACCELERATION_DUE_TO_GRAVITY, 0.0)

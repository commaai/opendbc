...
class LatControlCurvature:
  ...
  def update(self, CS, CC, desired_curvature):
    roll_compensation = float(getattr(CC, "rollCompensation", 0.0))
    steer_limited = bool(getattr(CC, "steerLimited", False))
    v_ego = CS.out.vEgo

    actual_curvature_vm = CC.currentCurvature  # includes roll
    yaw_rate = CC.angularVelocity[2] if len(CC.angularVelocity) > 2 else 0.0
    actual_curvature_pose = yaw_rate / max(v_ego, 0.1)
    actual_curvature = np.interp(v_ego, [2.0, 5.0], [actual_curvature_vm, actual_curvature_pose])
    desired_curvature_corr = desired_curvature - roll_compensation
    error = desired_curvature - actual_curvature
    freeze_integrator = steer_limited or v_ego < 5
    output_curvature = self.pid.update(error, feedforward=desired_curvature_corr, speed=v_ego,

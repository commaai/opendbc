...
def compute_gb_honda_nidec(accel, speed):
  ...
  if accel > 0:
    scale_factor = float ( np.interp ( speed, [0.0, 10.0, 20.0, 90.0], [7000.0, 12.0, 4.0, 1.0] ) )
    ...
    if not CC.longActive:
      ...
      if accel > 0:
        if CS.out.vEgo < 0.02: # disengage standstill
          ...
          self.speed_addon = max ( 0, self.speed_addon + float ( np.clip ( ( accel - CS.out.aEgo ) * 2, -0.005, 0.005 ) ) )
          pcm_speed = CS.out.vEgo + float ( np.clip ( self.speed_addon , 0, 5) )
          brake = 0.0
      else:

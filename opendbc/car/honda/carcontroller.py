...
class CarController(CarControllerBase):
  ...
  def update(self, CC, CS, now_nanos):
    ...
    conversion = hondacan.get_cruise_speed_conversion(self.CP.platform, CS.is_metric)
    hud_v_cruise = hud_control.setSpeed / conversion if hud_control.speedVisible else 255
    pcm_cancel_cmd = CC.cruiseControl.cancel

    if CC.longActive:
      accel = actuators.accel
      gas, brake = compute_gas_brake(actuators.accel, CS.out.vEgo, self.CP.platform)
    else:
      accel = 0.0
      gas, brake = 0.0, 0.0
    ...
    if self.CP.platform in (HONDA_BOSCH - HONDA_BOSCH_RADARLESS) and self.CP.openpilotLongitudinalControl:
      if self.frame % 10 == 0:
    ...
    if not CC.longActive:
      pcm_speed = 0.0
      pcm_accel = int(0.0)
    elif self.CP.platform in HONDA_NIDEC_ALT_PCM_ACCEL:
      pcm_speed_V = [0.0,
    ...
    if not self.CP.openpilotLongitudinalControl:
      if self.frame % 2 == 0 and self.CP.platform not in HONDA_BOSCH_RADARLESS:  # radarless cars don't have supplemental message
        can_sends.append(hondacan.create_bosch_supplemental_1(self.packer, self.CAN))
      # If using stock ACC, spam cancel command to kill gas when OP disengages.
      if pcm_cancel_cmd:
        can_sends.append(hondacan.spam_buttons_command(self.packer, self.CAN, CruiseButtons.CANCEL, self.CP.platform))
      elif CC.cruiseControl.resume:
        can_sends.append(hondacan.spam_buttons_command(self.packer, self.CAN, CruiseButtons.RES_ACCEL, self.CP.platform))
      ...
      if self.frame % 2 == 0:
        ...
        if self.CP.platform in HONDA_BOSCH:
          ...
          can_sends.extend(hondacan.create_acc_commands(self.packer, self.CAN, CC.enabled, CC.longActive, self.accel, self.gas,
                                                        self.stopping_counter, self.CP.platform))
        else:
          ...
          can_sends.append(hondacan.create_brake_command(self.packer, self.CAN, apply_brake, pump_on,
                                                         ...
                                                         self.CP.platform, CS.stock_brake))
    ...
    if self.frame % 10 == 0:
      ...
      if self.CP.openpilotLongitudinalControl and self.CP.platform not in HONDA_BOSCH:

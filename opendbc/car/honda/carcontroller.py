  ...
  def update(self, CC, CS, now_nanos):
    ...
    if not CC.longActive:
      ...
      pcm_speed_V = [0.0,
                     ...
                     np.clip(CS.out.vEgo + 5.0, 0.0, 100.0)]
      pcm_speed = float(np.interp(gas - brake, pcm_speed_BP, pcm_speed_V))
      pcm_accel = int(np.clip((accel / 1.44) * GAS_TUNE_FACTOR / max_accel, 0.0, 1.0 * GAS_MAX_FACTOR ) * self.params.NIDEC_GAS_MAX)
    ...
    if not self.CP.openpilotLongitudinalControl:
      ...
      if self.frame % 2 == 0:
        ...
        if self.CP.carFingerprint in HONDA_BOSCH:
          ...
          apply_brake = np.clip(self.brake_last - wind_brake, 0.0, BRAKE_MAX_FACTOR)
    ...
    if self.frame % 10 == 0:
      ...
      can_sends.extend(hondacan.create_ui_commands(self.packer, self.CAN, self.CP, CC.enabled, 0 if self.brake_last - wind_brake > 0 else pcm_speed, hud, CS.is_metric, CS.acc_hud, CS.lkas_hud))

      if self.CP.openpilotLongitudinalControl and self.CP.carFingerprint not in HONDA_BOSCH:
        self.speed = pcm_speed
        self.gas = int ( pcm_accel ) # / self.params.NIDEC_GAS_MAX

...
from opendbc.car.honda.values import HondaFlags, HONDA_BOSCH, HONDA_BOSCH_RADARLESS, HONDA_BOSCH_CANFD_CAR, CAR, CarControllerParams
...
class CanBus(CanBusBase):
  def __init__(self, CP=None, fingerprint=None) -> None:
    ...
    if CP.carFingerprint in (HONDA_BOSCH - HONDA_BOSCH_RADARLESS - HONDA_BOSCH_CANFD_CAR):
...
def get_cruise_speed_conversion(car_fingerprint: str, is_metric: bool) -> float:
  ...
  return CV.MPH_TO_MS if car_fingerprint in (HONDA_BOSCH_RADARLESS | HONDA_BOSCH_CANFD_CAR) and not is_metric else CV.KPH_TO_MS
...
def create_ui_commands(packer, CAN, CP, enabled, pcm_speed, hud, is_metric, acc_hud, lkas_hud):
  ...
  if CP.carFingerprint in HONDA_BOSCH_CANFD_CAR:
    lkas_hud_values['LANE_LINES'] = 3
    lkas_hud_values['DASHED_LANES'] = hud.lanes_visible
...
def spam_buttons_command(packer, CAN, button_val, car_fingerprint):
  ...
  bus = CAN.camera if car_fingerprint in (HONDA_BOSCH_RADARLESS | HONDA_BOSCH_CANFD_CAR) else CAN.pt

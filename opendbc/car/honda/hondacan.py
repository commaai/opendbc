...
def create_acc_commands(packer, CAN, enabled, active, aTarget, gas, stoppingDecelRate, stopping_counter, car_fingerprint, gas_force):
  commands = []
  min_gas_accel = CarControllerParams.BOSCH_GAS_LOOKUP_BP[0]

  control_on = 5 if enabled else 0
  gas_command = gas if active and gas_force > min_gas_accel else -30000
  accel_command = aTarget - stopping_counter * stoppingDecelRate if active else 0

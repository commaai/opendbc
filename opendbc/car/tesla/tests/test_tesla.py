...
# Hypothesized 3-bit DAS_steeringControlType profile, in terms of variant_code bookends (given software_major >= 4):
#   M3: variant_code starts with '4H',  ends with '015'
#   MY: variant_code starts with '4',   ends with '003'
# Older series (M3 '014', MY '002') are always the legacy 2-bit field.
DAS_STEERING_3_BIT_FW_RULE = {
...
class TestTeslaFingerprint(unittest.TestCase):
  ...
  def test_das_steering_3_bit_fw(self):
    for car_model, ecus in FW_VERSIONS.items():
      if car_model not in DAS_STEERING_3_BIT_FW_RULE:
        continue

      variant_prefix, variant_suffix = DAS_STEERING_3_BIT_FW_RULE[car_model]
      for fw in ecus.get((Ecu.eps, 0x730, None), []):
        m = FW_RE.match(fw)
        assert m is not None, f"Unparsable FW: {fw}"

        is_das_steering_3_bit = fw in DAS_STEERING_3_BIT_FW.get(car_model, [])
        expected = (
          m['variant_code'].startswith(variant_prefix)
          and m['variant_code'].endswith(variant_suffix)
          and int(m['software_major']) >= 4
        )
        assert is_das_steering_3_bit == expected, f"{fw}"

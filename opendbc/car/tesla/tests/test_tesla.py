...
from opendbc.car.tesla.interface import TeslaInterface as Interface
...
class TestTeslaFingerprint(unittest.TestCase):
  ...
  def test_radar_detection(self):
    ...
    for radar in (True, False):
      ...
      CP = Interface.get_params(CAR.TESLA_MODEL_3, fingerprint, [], False, False, False)
  ...
  def test_no_radar_car(self):
    ...
    for radar in (True, False):
      ...
      if radar:
        fingerprint[1][RADAR_START_ADDR] = 8
      CP = Interface.get_params(CAR.TESLA_MODEL_X, fingerprint, [], False, False, False)

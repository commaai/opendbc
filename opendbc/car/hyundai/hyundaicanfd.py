...
def create_ccnc(packer, CAN, CP, CC, CS, lat_active):
  ...
  msg_161.update({
    ...
    "LCA_LEFT_ICON": (
      1 if not lat_active or CS.out.leftBlindspot or CS.out.vEgo < 8.94 else 2
    ),
    "LCA_RIGHT_ICON": (
      1 if not lat_active or CS.out.rightBlindspot or CS.out.vEgo < 8.94 else 2
    ),
    ...
    "LANELINE_CURVATURE": curvature.get(max(-15, min(int(CS.out.steeringAngleDeg / 4.5), 15)), 14) if lat_active or enabled else 15,

...
IP_ADDRESS=$1
: "${IP_ADDRESS:?"The Loop server IP Address is expected as the first command line argument in order to run integration tests against the Loop server"}"
...
#python3 -m nose --with-xunit --xunit-file="${OUTPUT_DIR}/${IP_ADDRESS}-integration-nosetests.xml" -w python-tests
#check_bail_with_popd $?

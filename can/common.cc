...
unsigned int volkswagen_mqb_checksum(uint32_t address, const Signal &sig, const std::vector<uint8_t> &d) {
  ...
  switch(address) {
    case 0x86:  // LWI_01 Steering Angle
      ...
      break;
    case 0x97:  // ????
      crc ^= (uint8_t[]){0x3C,0x54,0xCF,0xA3,0x81,0x93,0x0B,0xC7,0x3E,0xDF,0x1C,0xB0,0xA7,0x25,0xD3,0xD8}[counter];
      break;
    ...
    case 0x122: // ACC_06 Automatic Cruise Control
      ...
      break;
    case 0x124: // ????
      crc ^= (uint8_t[]){0x12,0x7E,0x34,0x16,0x25,0x8F,0x8E,0x35,0xBA,0x7F,0xEA,0x59,0x4C,0xF0,0x88,0x15}[counter];
      break;
    ...
    case 0x187: // EV_Gearshift "Gear" selection data for EVs with no gearbox
      ...
      break;
    case 0x1A0: // ????
      crc ^= (uint8_t[]){0xCD,0x14,0x24,0x55,0x0D,0x60,0x1E,0x43,0x9A,0xAB,0xD5,0x53,0x12,0x19,0xA5,0xC3}[counter];
      break;
    case 0x1AC: // ????
      crc ^= (uint8_t[]){0xF2,0xA4,0x2A,0x3A,0xB4,0x3F,0x93,0xA8,0x1D,0x24,0xB3,0x32,0x19,0x38,0x5A,0x51}[counter];
      break;
    case 0x2C0: // ????
      crc ^= (uint8_t[]){0xE3,0xE7,0xA9,0x1F,0x27,0xA2,0xCF,0xC2,0x50,0x76,0xCD,0x25,0xD0,0x1A,0x60,0x30}[counter];
      break;      
    ...
    case 0x324: // ACC_04 Automatic Cruise Control
      ...
      break;
    case 0x3A6: // ????
      crc ^= (uint8_t[]){0xB6,0x1C,0xC1,0x23,0x6D,0x8B,0x0C,0x51,0x38,0x32,0x24,0xA8,0x3F,0x3A,0xA4,0x02}[counter];
      break;
    case 0x3AF: // ????
      crc ^= (uint8_t[]){0x94,0x6A,0xB5,0x38,0x8A,0xB4,0xAB,0x27,0xCB,0x22,0x88,0xEF,0xA3,0xE1,0xD0,0xBB}[counter];
      break;
    ...
    case 0x65D: // ESP_20 Electronic Stability Program
      ...
      break;
    case 0x6A3: // ????
      crc ^= (uint8_t[]){0xC1,0x8B,0x38,0xA8,0xA4,0x27,0xEB,0xC8,0xEF,0x05,0x9A,0xBB,0x39,0xF7,0x80,0xA7}[counter];
      break;
    case 0x6A4: // ????
      crc ^= (uint8_t[]){0xC7,0xD8,0xF1,0xC4,0xE3,0x5E,0x9A,0xE2,0xA1,0xCB,0x02,0x4F,0x57,0x4E,0x8E,0xE4}[counter];
      break;

...
std::vector<Msg *> CANParser::messages() const {
  std::vector<Msg *> result(message_states.size());
  std::transform(message_states.begin(), message_states.end(), result.begin(),
                 [this](auto &m) { return (Msg *)dbc->address_to_msg.at(m.first); });
  return result;
}

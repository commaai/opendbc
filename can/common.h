...
private:
  const DBC *dbc = NULL;
  std::unordered_map<uint32_t, const Msg *> message_address_lookup;
  std::unordered_map<std::string, const Msg *> message_name_lookup;
...
public:
  ...
  uint32_t addressFromName(const std::string &msg_name);

...
def find_include_files(file_in):
    split = include_pattern.split(file_in)

    return split[1::2], split[-1]

def replace_includes_recursively(src_dir, include_files):
  def process_include(include_filename):
    ret = '\n\nCM_ "Imported file %s starts here";\n' % include_filename
    
    child = read_dbc(src_dir, include_filename)

    include_files, core_dbc = find_include_files(child)

    ret += replace_includes_recursively(src_dir, include_files)

    ret += core_dbc

    return ret
  
  return ''.join([process_include(include_filename) for include_filename in include_files])
...
def create_dbc(src_dir: str, filename: str, output_path: str):
  ...
  with open(output_file_location, 'w', encoding='utf-8') as dbc_file_out:
    ...
    include_files, core_dbc = find_include_files(dbc_file_in)

    dbc_file_out.write(replace_includes_recursively(src_dir, include_files))
    dbc_file_out.write('\nCM_ "%s starts here";\n' % filename)
